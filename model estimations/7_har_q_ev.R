...
HarQEv <- function(df.rv = list.5.min$rvar
  ...
  df.har.q.ev <- data.frame(df.rv[, 1])
  names(df.har.q.ev) <- "date"
  ...
  for(j in 1:9){
    ...
    temp.df.har.q.ev <- data.frame(cbind(0.5 * log(df.rv[, name.y])                                                           #dependent variable
    ...
    names(temp.df.har.q.ev) <- c(paste0("rv.", var.tickers[j])
    ...
    temp.df.har.q.ev <- cleaner.df(temp.df.har.q.ev)
    df.har.q.ev <- cbind(df.har.q.ev, temp.df.har.q.ev)
    
    list.formula[[j]] <- formula(paste(paste(names(temp.df.har.q.ev)[1], paste(names(temp.df.har.q.ev)[-1], collapse = " + "), sep = " ~ ")))
  }
  
  y.names <- y.names[-1]
  
  if(!out.sample.mode){
    df.har.q.ev <- df.har.q.ev[523:n.df, ] ## use 500 to keep number of observations comparable to the hexp
  }
  
  if(out.sample.mode){
    
    df.har.q.ev.out <- df.har.q.ev[(training.sample.length + 1):n.df, ]
    df.har.q.ev <- df.har.q.ev[(1:training.sample.length), ]
  }
  
  #start estimation of the SUR
  mod.har.q.ev <- systemfit(list.formula, data = df.har.q.ev, method = "SUR")
  
  #To return the model, uncomment below
  # return(mod.har.q.ev)
  
  t.extract <- extract(mod.har.q.ev, beside = TRUE, include.rsquared = FALSE)
  ...
  # 
  # print(paste("ols r.squared", summary(mod.har.q.ev)$ols.r.squared))
  # print(paste("mcelroy r.squared", summary(mod.har.q.ev)$mcelroy.r.squared))
  # print(paste("log likelihood", logLik(mod.har.q.ev)))
  
  
  if(out.sample.mode){
    
    df.forecast <- predict(mod.har.q.ev, newdata = df.har.q.ev.out)
    df.real <- df.har.q.ev.out[, y.names]
  ...
  if(!out.sample.mode){
    ...
    df.forecast <- predict(mod.har.q.ev)
    df.errors <- residuals(mod.har.q.ev)
